...
export function setupDetailsPanel() {
  function createInfluencesChart(influences) {
    ...
    svg.selectAll(".bar").data(influences).enter().append("rect").attr("class","bar")
  ...
  return { update(genreData) {
      document.getElementById("genre-title").textContent=genreData["genre-name"];
      document.getElementById("genre-decade").textContent=`${genreData.origin["start-decade"]}s - ${genreData.origin.region}`;
