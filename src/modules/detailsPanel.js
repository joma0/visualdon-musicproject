...
export function setupDetailsPanel() {
  ...
  function updateSpotifyPlayer(genre) {
    ...
    const playlist =
      '<iframe style="border-radius:12px" src="https://open.spotify.com/embed/playlist/4fHgXcOxZjO97e3EuWL4P2?utm_source=generator&theme=0" width="100%" height="500" frameBorder="0" allowfullscreen="" allow="autoplay; clipboard-write; encrypted-media; fullscreen; picture-in-picture" loading="lazy"></iframe>';
    container.innerHTML = `
  ...
  return {
    update(genreData) {
      ...
      document.getElementById(
        "genre-decade"
      ).textContent = `${genreData.origin["start-decade"]}s - ${genreData.origin.region}`;
